function areas = explore_volume_negative_grayscaling(plane, starting_point, volume, dim)

    n= starting_point; template = im2double(extract_slice(volume, plane, n));
    ...
    min_gray_level = min(min(rect));
    max_gray_level = max(max(rect));

    pp_template = grayscaling_preprocessing(template, avg_gray_level, min_gray_level, max_gray_level, true);
    figure;
...
%         img = imcomplement(img);
        proc_img = grayscaling_preprocessing(img, avg_gray_level, min_gray_level, max_gray_level, false);
